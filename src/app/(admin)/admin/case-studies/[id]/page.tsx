...
import { ArrowLeft, Loader2, Save, Building2, BookOpen, Users, Clock, Target, Calendar, Eye, Trash2 } from "lucide-react"
  ...
  const fetchCaseStudy = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const found = caseStudies.find((c: CaseStudy) => c.id === caseStudyId)
        if (found) {
          setCaseStudy(found)
          setFormData({
            clientLabel: found.clientLabel,
            trainingTopic: found.trainingTopic,
            participantCount: found.participantCount,
            durationText: found.durationText || "",
            outcomeSummary: found.outcomeSummary || "",
            studyDate: found.studyDate ? new Date(found.studyDate).toISOString().split("T")[0] : "",
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 mt-1">Update case study details</p>
          </div>
        </div>
        <Button
          variant="outline"
